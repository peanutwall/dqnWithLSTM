...
class ForwardOneStep:
    ...
    def __call__(self, state, action):
        trueAction = np.array([np.cos(np.pi/4*action), np.sin(np.pi/4*action)])
        nextState = self.transitionFunction(state, trueAction)
        reward = self.rewardFunction(state, trueAction, nextState)
